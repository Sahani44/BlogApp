import 'package:blog_app/blog_categories/appDev/blog_pagead.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class BlogListAd extends StatefulWidget {
  const BlogListAd({ Key? key }) : super(key: key);

  @override
  _BlogListAdState createState() => _BlogListAdState();
}

class _BlogListAdState extends State<BlogListAd> {
  @override
...
class BlogsTileAd extends StatelessWidget {
  
  String imageurl = "",title = "",description = "",authorName = "",blogId = "";bool edit;

  BlogsTileAd({required this.imageurl,required this.title,required this.description,required this.authorName,required this.edit,required this.blogId});
  ...
  Widget build(BuildContext context) {
    return Material(
      child: InkWell(
        onTap: (){
          Navigator.push(context, MaterialPageRoute(builder: (context) => BlogPageAd(title: title,desc:description,imageUrl: imageurl,edit:edit,blogId:blogId)));
        },
        child: Stack(
          children: [
            SizedBox(
