import 'package:blog_app/blog_categories/webDev/blog_pagewd.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class BlogListWeb extends StatefulWidget {
  const BlogListWeb({ Key? key }) : super(key: key);

  @override
  _BlogListWebState createState() => _BlogListWebState();
}

class _BlogListWebState extends State<BlogListWeb> {
  @override
...
class BlogsTileWeb extends StatelessWidget {
  
  String imageurl = "",title = "",description = "",authorName = "",blogId = "";bool edit;

  BlogsTileWeb({required this.imageurl,required this.title,required this.description,required this.authorName,required this.edit,required this.blogId});
  ...
  Widget build(BuildContext context) {
    return Material(
      child: InkWell(
        onTap: (){
          Navigator.push(context, MaterialPageRoute(builder: (context) => BlogPageWeb(title: title,desc:description,imageUrl: imageurl,edit:edit,blogId:blogId)));
        },
        child: Stack(
          children: [
            SizedBox(
